...
$MsixPath    = "$ProjectDir\XboxLedControl.msix"
$CertPfxPath = "$ProjectDir\XboxLedDev.pfx"
$CertPass    = "xboxled123"
$Subject     = "CN=XboxLedDev"

# ── Helper: find SDK tool in Windows Kits, NuGet cache, or download ────────
function Get-SdkTool([string]$toolName) {
    # 1. Windows SDK (standard install)
    ...
    # 2. Already in local tools dir
    $local = "$ProjectDir\.sdktools\bin\10.0.22621.0\x64\$toolName"
    if (Test-Path $local) { return $local }

    # 3. NuGet cache (populated by VS / Windows App SDK)
...
function Download-SdkBuildTools {
    ...
    Write-Host "  makeappx/signtool not found — downloading Microsoft.Windows.SDK.BuildTools $version (~30 MB)..."
...
# ── 1. Publish ─────────────────────────────────────────────────────────────
Write-Host "[1/5] Publishing (self-contained win-x64)..."
if (Test-Path $PublishDir) { Remove-Item $PublishDir -Recurse -Force }
& "C:\Program Files\dotnet\dotnet.exe" publish "XboxLedControl.csproj" `
    -c Release -r win-x64 --self-contained true -o $PublishDir
if ($LASTEXITCODE -ne 0) { throw "dotnet publish failed" }

# ── 2. Stage package directory ─────────────────────────────────────────────
...
# Copy Devices.Abstraction.dll from Xbox Accessories App package
$xboxPkg = Get-AppxPackage -Name "Microsoft.XboxDevices" -ErrorAction SilentlyContinue
if ($xboxPkg) {
    $src = Join-Path $xboxPkg.InstallLocation "Devices.Abstraction.dll"
    if (Test-Path $src) {
        Copy-Item $src "$PkgStaging\Devices.Abstraction.dll" -Force
        Write-Host "  Copied Devices.Abstraction.dll from $($xboxPkg.InstallLocation)"
    } else {
        Write-Warning "  Devices.Abstraction.dll not found in Xbox Accessories App package!"
    }
} else {
    Write-Warning "  Microsoft.XboxDevices package not installed — Devices.Abstraction.dll will be missing!"
}

# Copy MSVC _APP runtime DLLs (required by Devices.Abstraction.dll)
$vclibsPkg = Get-AppxPackage -Name "Microsoft.VCLibs.140.00" |
...
if ($vclibsPkg) {
    foreach ($dll in @("msvcp140_app.dll", "vcruntime140_app.dll", "vcruntime140_1_app.dll")) {
        $src = Join-Path $vclibsPkg.InstallLocation $dll
        if (Test-Path $src) {
            Copy-Item $src "$PkgStaging\$dll" -Force
            Write-Host "  Copied $dll"
        }
    }
} else {
    Write-Warning "  Microsoft.VCLibs.140.00 (x64) not installed — _APP runtime DLLs missing!"
}

# Minimal 1x1 transparent PNG (required by manifest schema for logo assets)
...
if (-not (Test-Path $CertPfxPath)) {
    ...
    # Trust in Local Machine Trusted Root so Add-AppPackage accepts it
...
# ── 4. Locate or download SDK tools ────────────────────────────────────────
...
if (-not $makeappx) { throw "makeappx.exe not found even after download." }
if (-not $signtool)  { throw "signtool.exe not found even after download."  }
...
Write-Host ""
Write-Host "[OK] Installed. Usage (from any terminal):"
Write-Host "  XboxLedControl.exe --list"
Write-Host "  XboxLedControl.exe 50"
Write-Host "  XboxLedControl.exe off"
